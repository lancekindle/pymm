...
class BaseElement(metaclass=registry):
    ...
    def __getattr__(self, name):
        ...
        element. Check for and display helpful error messages that
        arise from pymm's slightly convoluted element inheritance and
        usage structure. In pymm, newer elements are used in place of
        the older element from which it inherits. This is done
        automatically. As soon as a user creates a new element class,
        it will be used (as applicable) when reading from a mindmap.
        Therefore, in this instance, it's possible the user has made
        the wrong assumption about which element is being used here.
        This function attempts to figure out which element the user
        MEANT to access (usually an older, inherited-from element),
        and includes that info when it re-raises AttributeError
        """
        registry.identify_attribute_error(self, name)
